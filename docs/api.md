...
- **Same set, different order** — a reseed. List position is the seeding, so `state.teams`
  ...
  transaction. Pools and the knockout bracket are then redrawn from that new order —
  seed order is what the serpentine draw uses to place teams into pools, so this uses the
  same generation and persistence path as a rebuild (fixtures deleted and regenerated,
  `state.rounds` rewritten, that division's entries removed from `tournaments.schedule`),
  just without adding or removing any team. Allowed only while the tournament is
  `Not Started` and the division holds no completed result — see `docs/decisions.md`,
...
| 409 | `TOURNAMENT_ALREADY_STARTED` — a rebuild or a reorder after the tournament has started, or `DIVISION_HAS_RESULTS` — a rebuild's or a reorder's division already holds a completed fixture |

A rebuild's and a reorder's gates are the same two checks, because both now delete and
regenerate fixtures: a status can simply be wrong and a completed fixture cannot, so
neither condition alone is enough. `num_groups` and `knockout_teams` are required on both
and neither is adjusted to fit: the organiser confirms them against the current team count
in the client, and correcting their choice without saying so is worse than refusing it.

Success returns `rebuilt` and `reordered`, the resulting team list in seed order, and the
